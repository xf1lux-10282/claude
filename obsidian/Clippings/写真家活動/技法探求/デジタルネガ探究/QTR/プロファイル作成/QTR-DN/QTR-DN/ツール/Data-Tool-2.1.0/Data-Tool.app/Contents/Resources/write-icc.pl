...
sub readdata
{
	...
	foreach my $dline (@inputarr) {
		...
		chomp($dline);
		$dline = uc($dline);
		$dline =~ s/,/ /g if $comma_flag;   #for .csv files -- comma separated values
		$dline =~ s/,/./g;			# convert commas and ignore leading/trailing spaces
		$dline =~ s/^\s*//;
		$dline =~ s/\s*$//;
		$dline =~ s/[#;].*//;		# ignore comments
		next unless $dline;

		my @arr = split (/\s+/ , $dline);
...
#	foreach my $a ( @datafile) { print $a, "\t", join("\t", @$a), "\n";	}
	...
	for (my $i=0; $i < $numcols; $i++) {
		...
		elsif (namein($hdr, ['LAB_A','A']))					{ $colname{'lab_a'} = $i; }
		elsif (namein($hdr, ['LAB_B','B']))					{ $colname{'lab_b'} = $i; }
...
#		print "max =\t", join("\t", @maxdata), "\n";
		...
		for my $i (0..$#maxdata) {
			...
			if ($mindata[$i] >= 0) {
				...
				elsif ($maxdata[$i] >= 90 && $maxdata[$i] < 100)
