#!/usr/bin/env python3
"""
PX1V-PtPd-xf1-v28-multiink-10pct QTRカーブ生成
v28ベース + GY/LGY 10%混合
...
- GY: K値の10%
- LGY: K値の10%
- 狙い: ディザリングパターン解消最強化（7%でも不十分だった場合）
...
print("=" * 80)
print("v28 マルチインク（GY+LGY 10%混合）カーブ生成開始")
print("=" * 80)
...
print(f"\n主要ポイントの確認:")
print(f"Input |    K | GY (10%) | LGY (10%)")
print(f"------|------|---------|-----------")
for i in [0, 8, 24, 48, 128, 192, 240, 255]:
    print(f"{i:5} | {k_values[i]:4} | {gy_values[i]:8} | {lgy_values[i]:9}")
...
# Title: PX1V-PtPd-xf1-v28-multiink-10pct
# Printer: EPSON Stylus Pro 3880
# PPD: EPSON Stylus Pro 3880
# - v28 base (v26's 0-254 + K[255]=40000)
# - Multi-ink: GY + LGY each 10% of K (increased from 3%)
# - Dithering pattern reduction (strongest)
...
# 保存
output_path = '/Users/daisukekinoshita/Library/Mobile Documents/iCloud~md~obsidian/Documents/obsidian/Clippings/写真家活動/技法探求/デジネガ/QTR/プロファイル作成/作業ファイル_現行版/PX1V-PtPd-xf1-v28-multiink-10pct.quad'
with open(output_path, 'w') as f:
    f.write(quad_content)

print(f"✓ v28-multiink-10pctカーブ生成完了: {output_path}")
...
# ===== まとめ =====
print(f"\n{'=' * 80}")
print(f"v28-multiink-10pct カーブ完成")
print(f"{'=' * 80}")
print(f"\n【仕様】")
print(f"  ✓ K値: v28と同一（v26の0-254 + K[255]=40000）")
print(f"  ✓ GY: K値の10%（LK チャンネル）← 3%から増量")
print(f"  ✓ LGY: K値の10%（LLK チャンネル）← 3%から増量")
print(f"  ✓ 露光時間: 6分05秒（v26の6分45秒から-40秒、-10%）")
print(f"\n【効果】")
print(f"  1. v28と同じ濃度バランス（紙白到達）")
print(f"  2. ディザリングパターン解消最強化（3%→10%）")
print(f"  3. 暖色傾向かなり強まる（注意）")
print(f"\n【インク配分（主要ポイント）】")
print(f"  Input   0: K=    0, GY=   0, LGY=   0")
print(f"  Input   8: K= 2520, GY= 252, LGY= 252 (3%時: 76)")
print(f"  Input 128: K=16380, GY=1638, LGY=1638 (3%時: 491)")
print(f"  Input 255: K=40000, GY=4000, LGY=4000 (3%時: 1200)")
print(f"\n【次のステップ】")
print(f"1. v28-multiink-10pctをQTRにインストール")
print(f"2. 6分05秒露光でテストプリント")
print(f"3. 3%版・5%版・7%版と比較してディザリング改善を評価")
print(f"4. 暖色化の程度を評価（10%では強すぎる可能性あり）")
print(f"{'=' * 80}")
