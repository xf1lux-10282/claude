...
print("=" * 80)
print("PX1V-PtPd-xf1-2ink カーブ生成開始")
print("=" * 80)
...
# Input 0-254: 30 K/step
for i in range(255):
    k_values_2ink.append(i * 30)

# Input 255: 40000 (紙白確実到達)
k_values_2ink.append(40000)

print(f"✓ K値計算完了: {len(k_values_2ink)} values")
print(f"  K[0] = {k_values_2ink[0]}")
print(f"  K[24] = {k_values_2ink[24]}")
print(f"  K[240] = {k_values_2ink[240]}")
print(f"  K[254] = {k_values_2ink[254]}")
print(f"  K[255] = {k_values_2ink[255]}")
...
for i in [0, 8, 24, 25, 48, 128, 240, 241, 254, 255]:
    total = k_values_2ink[i] + gy_values[i]
    k_pct = (k_values_2ink[i] / total * 100) if total > 0 else 0
    if i == 0:
        note = "シャドウ開始"
    elif i <= 24:
        note = "Zone 1"
        ...
        note = "紙白"
    print(f"{i:5} | {k_values_2ink[i]:5} | {gy_values[i]:7} | {total:5} | {k_pct:4.1f} | {note}")

# ===== 露光時間の設定 =====
print("\n[Step 4] 露光時間の設定")
# v29-doubleink-2と同じ: K=30/step
exposure_min = 5
exposure_sec = 27
...
# BOOST_K=0 - NO BOOST
# 2ink: K (density base) + GY (gradation)
#
# K curve
"""

for k in k_values_2ink:
    quad_content += f"{k}\n"
quad_content += "#\n"
...
# 保存
output_path = '/Users/daisukekinoshita/Library/Mobile Documents/iCloud~md~obsidian/Documents/obsidian/Clippings/写真家活動/技法探求/デジネガ/QTR/プロファイル作成/作業ファイル_現行版/PX1V-PtPd-xf1-2ink.quad'
with open(output_path, 'w') as f:
    f.write(quad_content)

print(f"✓ 2inkカーブ生成完了: {output_path}")
...
with open(csv_path, 'w', encoding='utf-8') as f:
    ...
    for i in range(256):
        ...
        f.write(f"{i},{k_values_2ink[i]},{gy_values[i]},{total},{k_pct:.2f},{gy_pct:.2f}\n")

print(f"✓ CSV出力完了: {csv_path}")

# ===== まとめ =====
print(f"\n{'=' * 80}")
print(f"PX1V-PtPd-xf1-2ink カーブ完成")
print(f"{'=' * 80}")
print(f"\n【仕様】")
...
print(f"  4. ディザリングパターン解消（K+GY混合、K配分15-20%）")
print(f"\n【インク配分（主要ポイント）】")
