#!/usr/bin/env python3
"""
SP1v19カーブ生成スクリプト - ハイライト諧調拡張版（修正版）

修正方針:
V18をベースに、ハイライト部分(Input 224-255)の諧調を拡張
- Input 0-223: V18のまま（コントラスト削減効果を維持）
- Input 224-255: ハイライトストレッチ（修正版）
  - 224 → V18[224] (0.99D) ← V18より濃くする
  - 232 → V18[232] (1.06D) の代わりに中間値
  - 240 → 1.15D 付近
  - 248 → 1.20D 付近
  - 255 → 1.25D
...
- Input 223→224で濃度が下がらないように修正
- 0.99D-1.25Dの範囲に32ステップを配置
...
def create_sp1v19_quads():
    ...
    anchor_densities = [
        quad_to_density(v18_quads[224]),  # 224 → V18[224] (0.99D) ← 修正
        1.05,   # 232 → 1.05D
        1.12,   # 240 → 1.12D
        1.19,   # 248 → 1.19D
        1.25    # 255 → 1.25D
    ...
    anchor_quads = [density_to_quad(d) for d in anchor_densities]

    print(f"\nアンカーポイント（修正版）:")
    print(f"  Input 223 (V18): Quad={v18_quads[223]:5d}, Density={quad_to_density(v18_quads[223]):.4f}D")
    for inp, quad, dens in zip(anchor_inputs, anchor_quads, anchor_densities):
...
def write_quad_file(quads, filename):
    ...
    with open(filename, 'w') as f:
        ...
        f.write('# PX1V-PtPd-SP1v19 - Highlight tonal extension (corrected)\n')
        ...
        f.write('#   224 → V18[224] (0.99D)\n')
        f.write('#   232 → 1.05D\n')
        f.write('#   240 → 1.12D\n')
        f.write('#   248 → 1.19D\n')
        f.write('#   255 → 1.25D\n')
        f.write('# Created: 2026-03-19 SP1v19 (corrected)\n')
...
def plot_comparison(v18_quads, v19_quads):
    ...
    ax1.plot(inputs, v19_quads, 'r-', label='V19 (Highlight extended - corrected)', linewidth=2)
    ...
    ax1.set_title('V18 vs V19 (Corrected) - Highlight Tonal Extension', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.plot(inputs, v19_density, 'r-', label='V19 (Corrected)', linewidth=2)

    ax2.axvline(x=224, color='orange', linestyle='--', alpha=0.5)
    ax2.axhspan(0.99, 1.25, color='orange', alpha=0.1, label='V19 Highlight Range')
    ...
    ax3.plot(highlight_inputs, v19_highlight, 'r-^', label='V19 (Corrected)', linewidth=2, markersize=3)

    ax3.axvline(x=224, color='orange', linestyle='--', alpha=0.5)
    ax3.axhline(y=0.99, color='red', linestyle=':', alpha=0.3, label='0.99D')
    ...
    base_path = "/Users/daisukekinoshita/Library/Mobile Documents/iCloud~md~obsidian/Documents/obsidian/Clippings/写真家活動/技法探求/デジネガ/QTR/プロファイル作成/作業ファイル_現行版"
    plt.savefig(f"{base_path}/SP1v19_corrected_preview.png", dpi=150, bbox_inches='tight')

    print(f"\n✓ グラフ保存: SP1v19_corrected_preview.png")

    plt.close()

def main():
    print("=" * 80)
    print("SP1v19カーブ生成開始（ハイライト諧調拡張版・修正版）")
    print("=" * 80)
    ...
    # サマリー
    print("\n" + "=" * 80)
    print("SP1v19カーブ生成完了（修正版）")
    print("=" * 80)

    print(f"\n【修正内容】")
    print(f"✓ Input 0-223: V18のまま（コントラスト削減-0.05D維持）")
    print(f"✓ Input 224-255: ハイライトストレッチ（0.99D-1.25D）")
    print(f"  - Input 223→224で濃度が上昇（単調増加保証）")
    print(f"  - 32ステップで約0.26Dをカバー")
    print(f"  - 各ステップ約0.0081D")

    print(f"\n【期待される効果】")
    print(f"✓ Kプリント（Pt/Pd）のK3-15%の諧調を滑らかに表現")
    print(f"✓ ハイライトのディテール保持")
    print(f"✓ Input 223→224で濃度が下がらない")
