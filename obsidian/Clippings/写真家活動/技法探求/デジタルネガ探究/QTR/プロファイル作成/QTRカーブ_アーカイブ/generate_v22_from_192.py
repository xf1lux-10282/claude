#!/usr/bin/env python3
"""
PX1V-PtPd-v22: Input 192から調整版

重要な変更点:
- Input 0-184: v18のQuad値を完全保持
- Input 192-255: 新しい目標値で調整
- 248: 1.23D, 255: 1.25D（境界線リスク最小化）
"""
...
def generate_v22_from_192():
    """v22カーブを生成 - Input 192から調整"""
    print("=" * 80)
    print("PX1V-PtPd-v22: Input 192から調整版")
    print("=" * 80)
    ...
    print("  ✓ Input 192-255: v22目標濃度から計算")
    print("  ✓ 各アンカーポイント間: 線形補間で滑らかに接続")

    print("\nv18実測値（Input 176-216）:")
    for inp in [176, 184, 192, 200, 208, 216]:
        v18_val = v18_measured[v18_measured['input'] == inp]['negative_density'].values[0]
        print(f"  Input {inp}: {v18_val:.2f}D")

    print("\nv21実測値（Input 192-255）:")
    for inp in [192, 200, 208, 216, 224, 232, 240, 248, 255]:
    ...
    v22_targets = {
        ...
        224: 1.11,  # v21と同じ
        232: 1.15,  # v21と同じ
        240: 1.20,  # v21: 1.25 → -0.05D
        248: 1.23,  # v21: 1.32 → -0.09D
        255: 1.25,  # v21: 1.34 → -0.09D（紙白目標）
    }

    print("\nv22目標値:")
    print(f"{'Input':<6} {'v22目標':<10} {'v18実測':<10} {'v21実測':<10} {'v18比':<10} {'v21比':<10}")
    print("-" * 60)
    for inp in [192, 200, 208, 216, 224, 232, 240, 248, 255]:
        v22_target = v22_targets[inp]
        v18_val = v18_measured[v18_measured['input'] == inp]['negative_density'].values[0]
        v21_val = v21_measured[v21_measured['input'] == inp]['negative_density'].values[0]
        v18_diff = v22_target - v18_val
        v21_diff = v22_target - v21_val
        print(f"{inp:<6} {v22_target:<10.2f} {v18_val:<10.2f} {v21_val:<10.2f} {v18_diff:+.2f}      {v21_diff:+.2f}")

    # 勾配チェック
    print("\n勾配チェック（境界線リスク評価）:")
    anchor_list = [184, 192, 200, 208, 216, 224, 232, 240, 248, 255]
    for i in range(len(anchor_list) - 1):
        start_inp = anchor_list[i]
        end_inp = anchor_list[i + 1]

        if start_inp == 184:
            start_dens = v18_measured[v18_measured['input'] == start_inp]['negative_density'].values[0]
        else:
            start_dens = v22_targets[start_inp]

        end_dens = v22_targets[end_inp]

        diff = end_dens - start_dens
        num_points = end_inp - start_inp
        avg_gradient = diff / num_points
        quad_gradient_estimate = avg_gradient * SLOPE * 257

        status = "✓" if quad_gradient_estimate < 200 else "⚠️"
        print(f"  {status} {start_inp}→{end_inp}: {diff:.3f}D / {num_points}点 = {avg_gradient:.5f}D/点 (Quad勾配≈{quad_gradient_estimate:.0f})")

    # v22のQuad値を構築
    v22_quad_values = np.copy(v18_quad_values)

    # アンカーポイント（184 + v22の192-255）
    anchor_inputs = [184] + list(v22_targets.keys())
    anchor_quads = []

    # Input 184: v18のQuad値
    anchor_quads.append(v18_quad_values[184])

    # Input 192-255: v22目標濃度から計算
    for inp in v22_targets.keys():
    ...
    for i, inp in enumerate(anchor_inputs):
        if inp == 184:
            ...
            print(f"  Input {inp}: Quad={anchor_quads[i]} (v18: {v18_dens:.2f}D)")
        else:
            print(f"  Input {inp}: Quad={anchor_quads[i]} (目標: {v22_targets[inp]:.2f}D)")
    ...
    if max_diff > 200:
        ...
        # ハイライト域（192-255）の問題
        highlight_problems = [i for i in range(len(quad_diffs)) if quad_diffs[i] > 200 and i >= 192]
        v18_highlight_problems = [i for i in range(len(v18_quad_diffs)) if v18_quad_diffs[i] > 200 and i >= 192]

        print(f"  調整範囲（192-255）: {len(highlight_problems)}箇所（v18: {len(v18_highlight_problems)}箇所）")

        # v22で新規発生
        new_problems = []
        for i in range(192, 256):
            if i < len(quad_diffs) and quad_diffs[i] > 200:
                if i >= len(v18_quad_diffs) or v18_quad_diffs[i] <= 200:
                    new_problems.append(i)

        if new_problems:
            print(f"  ⚠️  v22で調整範囲に新規発生: {len(new_problems)}箇所")
            for idx in new_problems[:10]:
                print(f"    Input {idx:3d}→{idx+1:3d}: v22={quad_diffs[idx]:.1f}, v18={v18_quad_diffs[idx]:.1f}")
            if len(new_problems) > 10:
                print(f"    ... 他 {len(new_problems)-10}箇所")
        else:
    ...
    # ハイライト域のQuad差分詳細
    print("\n調整範囲のQuad差分（主要ポイント）:")
    check_points = [184, 191, 192, 199, 200, 207, 208, 215, 216, 223, 224, 231, 232, 239, 240, 247, 248, 254]
    ...
    print("\nv22予想濃度（逆算）:")
    for inp in [176, 184, 192, 200, 208, 216, 224, 232, 240, 248, 255]:
    ...
    # ハイライト範囲
    highlight_range = densities[255] - densities[240]
    print(f"\nハイライト範囲（240-255）: {highlight_range:.4f}D")
    ...
    with open(output_quad, 'w') as f:
        ...
        f.write('# PX1V-PtPd-v22 - Adjusted from Input 192\n')
        f.write('# Based on v18 Quad values (Input 0-184 unchanged)\n')
        f.write('# Input 185-255: Linear interpolation between anchor points\n')
        f.write('# Anchors: 192: 0.94, 200: 0.99, 208: 1.04, 216: 1.07\n')
        f.write('#          224: 1.11, 232: 1.15, 240: 1.20, 248: 1.23, 255: 1.25\n')
        f.write('# Highlight range 240-255: 0.05D\n')
    ...
    # v22アンカーポイント
    v22_anchor_inputs = list(v22_targets.keys())
    ...
    axes[0, 0].set_title('v22: Adjusted from Input 192',
    ...
    axes[0, 1].set_title('v22: Smooth Transition from 192',
                         fontsize=14, fontweight='bold')
    axes[0, 1].legend()
    ...
    axes[1, 1].set_title('v22: Adjusted Range Quad Differences (176-255)',
                         fontsize=14, fontweight='bold')
    axes[1, 1].legend()
    ...
    plt.tight_layout()
    plt.savefig('v22_from_192_analysis.png', dpi=300, bbox_inches='tight')
    print(f"✓ グラフ保存: v22_from_192_analysis.png")
...
if __name__ == '__main__':
    quad_values, densities = generate_v22_from_192()

    print("\n" + "=" * 80)
    print("v22 カーブ生成完了 - Input 192から調整")
    print("境界線リスク最小化、ハイライト範囲確保")
    print("=" * 80)
