#!/usr/bin/env python3
"""
SP1v22 QTR Curve Generator - FIXED VERSION
==========================================

CRITICAL FIX: QTR K values are 0-65535 (16-bit), not 0-255!
...
def load_sp1v21_quad():
    ...
    # Find K channel start (after "# K curve" comment)
    k_start = None
    ...
    if k_start is None:
        raise ValueError("Cannot find '# K curve' marker in .quad file")

    # Read 256 K values
    k_values = []
    for i in range(k_start, min(k_start + 256, len(lines))):
        line = lines[i].strip()
        if line and line.isdigit():
            k_values.append(int(line))
        elif line.startswith('#'):
            # Hit next channel, stop
            break
    ...
    print(f"  K[0] = {k_values[0]}, K[128] = {k_values[128]}, K[255] = {k_values[255]}")

    return np.arange(256), np.array(k_values)

def load_measurement_7m():
    """Load 7min exposure measurement data"""
    print("\n📊 Loading 7min measurement data...")
...
def k_to_negative_density(k_values, measurement_df):
    """
    Convert K values (0-65535) to negative density using SP1v21 measurement calibration

    SP1v21 was measured at 7min exposure, so we use that data to calibrate.
    """
    print("\n🔄 Converting SP1v21 K values to negative density...")

    # Get measured K values at known inputs
    measured_inputs = measurement_df['input'].values
    measured_neg_density = measurement_df['negative_density_transmission'].values

    # Get K values at those same inputs
    measured_k = k_values[measured_inputs]

    # Create K → density mapping using measured data
    # Sort by K value for interpolation
    sort_idx = np.argsort(measured_k)
    sorted_k = measured_k[sort_idx]
    sorted_density = measured_neg_density[sort_idx]

    # Create interpolation function: K → density
    f_k_to_d = interpolate.interp1d(
        sorted_k,
        sorted_density,
        kind='cubic',
    ...
    # Apply to all 256 K values
    neg_density = f_k_to_d(k_values)

    # Clamp to reasonable range
    neg_density = np.clip(neg_density, 0.0, 2.0)

    print(f"✓ Converted K values to negative density")
    print(f"  Density range: {neg_density.min():.3f}D - {neg_density.max():.3f}D")
...
def negative_density_to_k(neg_density, sp1v21_k_values, sp1v21_neg_density):
    """
    Convert negative density to K values (0-65535) using SP1v21 calibration

    Strategy: Use SP1v21's K-to-density mapping as reference
    """
    print("\n🔄 Converting SP1v22 negative density to K values...")

    # Create density → K mapping from SP1v21
    # Sort by density for interpolation
    sort_idx = np.argsort(sp1v21_neg_density)
    sorted_density = sp1v21_neg_density[sort_idx]
    sorted_k = sp1v21_k_values[sort_idx]

    # Create interpolation function: density → K
...
def write_quad_file(k_values, output_path):
    ...
    # K channel values (256 values, 0-65535 range)
    for val in k_values:
    ...
    print(f"  K channel: 256 values (range {min(k_values)}-{max(k_values)})")
    print(f"  Other channels: 9 × 256 zeros")
...
def create_comparison_graph(sp1v21_neg, sp1v22_neg, measurement_df, output_path):
    ...
    fig.suptitle('SP1v22 Curve Comparison (FIXED)', fontsize=16, fontweight='bold')
...
def analyze_banding_risk(sp1v22_neg):
    ...
    for i in range(len(sp1v22_neg) - 1):
        diff = sp1v22_neg[i+1] - sp1v22_neg[i]
    ...
    if banding_issues:
        ...
        for issue in banding_issues[:10]:
...
def main():
    """Main execution"""
    print("=" * 70)
    print("SP1v22 CURVE GENERATOR (FIXED VERSION)")
    print("=" * 70)
    print(f"CRITICAL FIX: QTR K values are 0-65535, not 0-255!")
    ...
    # Convert back to K values (FIXED: now uses 0-65535 range)
    sp1v22_k_values = negative_density_to_k(sp1v22_neg, sp1v21_k_values, sp1v21_neg)
    ...
    # Summary
    print("\n" + "=" * 70)
    print("SP1v22 GENERATION COMPLETE (FIXED)")
    print("=" * 70)
    ...
    print("4. Apply Rule 8: xattr -c + cache clear")
    print("5. Print test chart with SP1v22 @ 7min exposure")
    print("6. Measure and verify results")
    print("=" * 70)
