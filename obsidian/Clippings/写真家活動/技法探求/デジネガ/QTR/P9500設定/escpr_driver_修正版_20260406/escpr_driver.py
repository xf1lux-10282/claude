...
class ESCPRDriver:
    ...
    def start_job(self, width_mm=210, height_mm=297, dpi_x=2880, dpi_y=1440,
                  color_mode=1, color_plane=1, page_setup=None):
        """
        ジョブ開始
        ...
        Args:
            ...
            page_setup: ページセットアップID (EPIJ_PSrc)
                5=Sheet, 14=Roll Paper, 74=Poster Board
                Noneの場合はコマンドを送信しない
        """
        # ページセットアップ（用紙給紙モード）を最初に設定
        if page_setup is not None:
            self.send(make_page_setup_command(page_setup))
