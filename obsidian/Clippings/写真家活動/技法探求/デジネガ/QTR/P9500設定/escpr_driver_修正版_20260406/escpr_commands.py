...
# ==============================================================================
# ページセットアップコマンド (EPIJ_PSrc)
# ==============================================================================

def make_page_setup_command(page_setup=5):
    """
    ページセットアップコマンド生成 (EPIJ_PSrc)

    Args:
        page_setup: ページセットアップID
            5  = Sheet (シート給紙)
            51 = Sheet - Centering
            7  = Sheet - Borderless Auto Expand
            8  = Sheet - Borderless Retain Size
            14 = Roll Paper (ロール紙)
            18 = Roll Paper - Borderless Auto Expand
            19 = Roll Paper - Borderless Retain Size
            16 = Roll Paper - Banner
            74 = Poster Board
            75 = Poster Board - Centering

    Returns:
        bytes: ESC j "setp" コマンド

    Note:
        EPSONのEPIJ_PSrcパラメータに対応。
        用紙給紙モードをプリンターに指示する。
    """
    # ESC/P-Rコマンド形式: ESC j "setp" <page_setup_id>
    # page_setup_idは1バイト
    data = struct.pack('<B', page_setup)

    return _make_escpr_command('j', 'setp', data)
