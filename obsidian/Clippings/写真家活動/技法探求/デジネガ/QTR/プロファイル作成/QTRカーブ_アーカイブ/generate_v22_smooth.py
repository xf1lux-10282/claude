#!/usr/bin/env python3
"""
PX1V-PtPd-v22: v18のQuad値を使用 + ハイライト域を滑らかに補間

戦略:
- Input 0-216: v18の.quadファイルからQuad値をそのまま使用
- Input 224, 232, 240, 248, 255: v22目標濃度から計算（アンカーポイント）
- Input 217-223, 225-231, 233-239, 241-247, 249-254: 線形補間
"""
...
def generate_v22_smooth():
    """v18のQuad値ベースでv22を生成 - 全範囲を滑らかに補間"""
    print("=" * 80)
    print("PX1V-PtPd-v22: v18 Quad値ベース + 滑らかなハイライト補間")
    print("=" * 80)
    ...
    for inp in [224, 232, 240, 248, 255]:
        ...
        print(f"  Input {inp}: v22目標={v22_target:.2f}D (v21: {v21_val:.2f}D, {diff:+.2f}D)")
    ...
    for i, inp in enumerate(anchor_inputs):
        print(f"  Input {inp}: Quad={anchor_quads[i]}")
    ...
    for i in range(len(anchor_inputs) - 1):
        ...
        print(f"  Input {start_input}→{end_input}: {num_points}点補間（平均勾配: {avg_diff:.1f}）")
    ...
    if max_diff > 200:
        ...
        if new_problems:
            ...
            for idx in new_problems[:15]:
                print(f"    Input {idx:3d}→{idx+1:3d}: Quad差分 {quad_diffs[idx]:.1f}")
            if len(new_problems) > 15:
                print(f"    ... 他 {len(new_problems)-15}箇所")
        else:
    ...
    for i in range(216, 255):
        if i in [216, 217, 223, 224, 225, 231, 232, 233, 239, 240, 241, 247, 248, 249, 254]:
            diff = quad_diffs[i]
    ...
    with open(output_quad, 'w') as f:
        ...
        f.write('# Anchors: 224: 1.06, 232: 1.10, 240: 1.15, 248: 1.20, 255: 1.22\n')
    ...
    axes[0, 0].set_title('v22: Smooth Interpolation (v18 Base + v22 Highlight)',
    ...
    axes[0, 1].set_title('v22: Smooth Highlight Interpolation',
                         fontsize=14, fontweight='bold')
    axes[0, 1].legend()
    ...
    plt.tight_layout()
    plt.savefig('v22_smooth_analysis.png', dpi=300, bbox_inches='tight')
    print(f"✓ グラフ保存: v22_smooth_analysis.png")
...
if __name__ == '__main__':
    quad_values, densities = generate_v22_smooth()

    print("\n" + "=" * 80)
    print("v22 カーブ生成完了 - 滑らかな補間版")
    print("Input 0-216: v18のQuad値（完全保持）")
    print("Input 217-255: 線形補間で滑らかに接続")
    print("=" * 80)
