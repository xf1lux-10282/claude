#!/usr/bin/env python3
"""
PX1V-PtPd-xf1-v28-multiink-7pct QTRカーブ生成
v28ベース + GY/LGY 7%混合
...
- GY: K値の7%
- LGY: K値の7%
- 狙い: ディザリングパターン解消強化（5%でも不十分だった場合）
...
print("=" * 80)
print("v28 マルチインク（GY+LGY 7%混合）カーブ生成開始")
print("=" * 80)
...
# 保存
output_path = '/Users/daisukekinoshita/Library/Mobile Documents/iCloud~md~obsidian/Documents/obsidian/Clippings/写真家活動/技法探求/デジネガ/QTR/プロファイル作成/作業ファイル_現行版/PX1V-PtPd-xf1-v28-multiink-7pct.quad'
with open(output_path, 'w') as f:
    f.write(quad_content)

print(f"✓ v28-multiink-7pctカーブ生成完了: {output_path}")
...
# ===== まとめ =====
print(f"\n{'=' * 80}")
print(f"v28-multiink-7pct カーブ完成")
print(f"{'=' * 80}")
print(f"\n【仕様】")
print(f"  ✓ K値: v28と同一（v26の0-254 + K[255]=40000）")
print(f"  ✓ GY: K値の7%（LK チャンネル）← 3%から増量")
print(f"  ✓ LGY: K値の7%（LLK チャンネル）← 3%から増量")
print(f"  ✓ 露光時間: 6分05秒（v26の6分45秒から-40秒、-10%）")
print(f"\n【効果】")
print(f"  1. v28と同じ濃度バランス（紙白到達）")
print(f"  2. ディザリングパターン解消強化（3%→7%）")
print(f"  3. 暖色傾向強まる")
print(f"\n【インク配分（主要ポイント）】")
print(f"  Input   0: K=    0, GY=   0, LGY=   0")
print(f"  Input   8: K= 2520, GY= 176, LGY= 176 (3%時: 76)")
print(f"  Input 128: K=16380, GY=1147, LGY=1147 (3%時: 491)")
print(f"  Input 255: K=40000, GY=2800, LGY=2800 (3%時: 1200)")
print(f"\n【次のステップ】")
print(f"1. v28-multiink-7pctをQTRにインストール")
print(f"2. 6分05秒露光でテストプリント")
print(f"3. 3%版・5%版と比較してディザリング改善を評価")
print(f"{'=' * 80}")
