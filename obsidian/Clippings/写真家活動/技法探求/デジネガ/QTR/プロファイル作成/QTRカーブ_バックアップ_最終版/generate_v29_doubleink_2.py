#!/usr/bin/env python3
"""
PX1V-PtPd-xf1-v29-doubleink-2 QTRカーブ生成
K + GY ダブルインク構成（K値強化版）
...
- K channel (濃度ベース):
  - Input 0-254: 30 K/step (v29-doubleinkの10 K/stepから3倍)
  - Input 255: 40000 (紙白確実到達)

狙い:
- GYでv29と同じ豊かな階調を確保
- Kを強化（30 K/step）して濃度ベースを増強
- ディザリングパターン解消
...
print(f"  v29 K[0] = {v29_k_values[0]}")
print(f"  v29 K[24] = {v29_k_values[24]}")
print(f"  v29 K[240] = {v29_k_values[240]}")
print(f"  v29 K[254] = {v29_k_values[254]}")
print(f"  v29 K[255] = {v29_k_values[255]}")
...
print("\n[Step 3] K値を計算（0-254は30 K/step）")

k_values_doubleink2 = []

# Input 0-254: 30 K/step
for i in range(255):
    k_values_doubleink2.append(i * 30)

# Input 255: 40000 (紙白確実到達)
k_values_doubleink2.append(40000)

print(f"✓ K値計算完了: {len(k_values_doubleink2)} values")
print(f"  K[0] = {k_values_doubleink2[0]}")
print(f"  K[24] = {k_values_doubleink2[24]}")
print(f"  K[240] = {k_values_doubleink2[240]}")
print(f"  K[254] = {k_values_doubleink2[254]}")
print(f"  K[255] = {k_values_doubleink2[255]}")
...
for i in [0, 8, 24, 25, 48, 128, 240, 241, 254, 255]:
    total = k_values_doubleink2[i] + gy_values[i]
    k_pct = (k_values_doubleink2[i] / total * 100) if total > 0 else 0
    if i == 0:
        note = "シャドウ開始"
    elif i <= 24:
        note = "Zone 1"
        ...
        note = "紙白"
    print(f"{i:5} | {k_values_doubleink2[i]:5} | {gy_values[i]:7} | {total:5} | {k_pct:4.1f} | {note}")

# ===== 露光時間の計算 =====
print("\n[Step 5] 露光時間の計算")

# v29と同じGY、Kが3倍なので、露光時間は少し短縮
# 概算: v29の6分04秒から約10%短縮
v29_exposure_sec = 6 * 60 + 4  # 364秒
exposure_reduction_pct = 10
doubleink2_exposure_sec = v29_exposure_sec * (1 - exposure_reduction_pct / 100)
exposure_min = int(doubleink2_exposure_sec // 60)
exposure_sec = int(doubleink2_exposure_sec % 60)

print(f"  v29: 6分04秒")
print(f"  v29-doubleink-2: {exposure_min}分{exposure_sec:02d}秒 (-{exposure_reduction_pct}%)")
...
# Title: PX1V-PtPd-xf1-v29-doubleink-2
# Printer: EPSON Stylus Pro 3880
# PPD: EPSON Stylus Pro 3880
# - Double ink configuration: K + GY(LK), K enhanced version
...
#   - Input 0-254: 30 K/step (3x of v29-doubleink's 10 K/step)
...
# - Benefit: Rich gradation (GY=v29) + stronger density base (K=30/step)
# Generated: 2026-04-04 by Claude Code

# 2026 Daisuke Kinoshita
# BOOST_K=0 - NO BOOST
# Double ink K enhanced: GY (v29 gradation) + K (30/step density)
#
# K curve
"""

for k in k_values_doubleink2:
    quad_content += f"{k}\n"
quad_content += "#\n"
...
# 保存
output_path = '/Users/daisukekinoshita/Library/Mobile Documents/iCloud~md~obsidian/Documents/obsidian/Clippings/写真家活動/技法探求/デジネガ/QTR/プロファイル作成/作業ファイル_現行版/PX1V-PtPd-xf1-v29-doubleink-2.quad'
with open(output_path, 'w') as f:
    f.write(quad_content)

print(f"✓ v29-doubleink-2カーブ生成完了: {output_path}")
...
with open(csv_path, 'w', encoding='utf-8') as f:
    ...
    for i in range(256):
        ...
        k_step = "30/step" if i <= 254 else "40000"
        f.write(f"{i},{k_values_doubleink2[i]},{gy_values[i]},{total},{k_pct:.2f},{gy_pct:.2f},{k_step}\n")

print(f"✓ CSV出力完了: {csv_path}")

# ===== まとめ =====
print(f"\n{'=' * 80}")
print(f"v29-doubleink-2 カーブ完成")
print(f"{'=' * 80}")
print(f"\n【仕様】")
...
print(f"    - Input 0-254: 30 K/step (v29-doubleinkの3倍)")
...
print(f"  2. 濃度ベース強化（K=30/step、v29-doubleinkの3倍）")
